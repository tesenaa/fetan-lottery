...
export default function App() {
  ...
  const [sysSettings, setSysSettings] = useState({ ticketPrice: 10, winnerPercentage: 80, manualWinningNumber: null });
  const [manualNumberInput, setManualNumberInput] = useState('');

  // 1 እስከ 1000 ያሉትን ቁጥሮች በአንድ ላይ ማዘጋጀት
          ...
          <>
            ...
            {currentScreen === 'board' && (
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                {/* Header Navigation */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', backgroundColor: '#0a0a16', borderBottom: '1px solid #1e1b4b', flexShrink: 0 }}>
                ...
                {/* Header Stats */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px', padding: '8px', flexShrink: 0 }}>
                  <div style={{ backgroundColor: '#1e1b4b', padding: '8px 2px', borderRadius: '6px', textAlign: 'center' }}>
                    <div style={{ fontSize: '9px', color: '#9ca3af' }}>Game ID</div>
                    <div style={{ fontSize: '11px', fontWeight: 'bold' }}>Fetan-001</div>
                  </div>
                  <div style={{ backgroundColor: '#1e1b4b', padding: '8px 2px', borderRadius: '6px', textAlign: 'center' }}>
                    <div style={{ fontSize: '9px', color: '#9ca3af' }}>Players</div>
                    <div style={{ fontSize: '11px', fontWeight: 'bold', color: '#38bdf8' }}>{playerCount}</div>
                  </div>
                  <div style={{ backgroundColor: '#1e1b4b', padding: '8px 2px', borderRadius: '6px', textAlign: 'center' }}>
                    <div style={{ fontSize: '9px', color: '#9ca3af' }}>Stake</div>
                    <div style={{ fontSize: '11px', fontWeight: 'bold', color: '#eab308' }}>{stake} ETB</div>
                  </div>
                  <div style={{ backgroundColor: '#1e1b4b', padding: '8px 2px', borderRadius: '6px', textAlign: 'center' }}>
                    <div style={{ fontSize: '9px', color: '#9ca3af' }}>Derash</div>
                    <div style={{ fontSize: '11px', fontWeight: 'bold', color: '#22c55e' }}>{derash} ETB</div>
                  </div>
                </div>

                {/* Timer Banner */}
                <div style={{ padding: '0 8px 6px 8px' }}>
                  <div style={{ backgroundColor: phase === 'spinning' ? (allPickedNumbers.length > 0 ? '#dc2626' : '#6b7280') : '#0284c7', padding: '6px', borderRadius: '6px', textAlign: 'center', fontSize: '12px', fontWeight: 'bold' }}>
                    {phase === 'spinning' ? (allPickedNumbers.length > 0 ? '🎰 እጣ እየወጣ ነው...' : '⚠️ ምንም ቁጥር አልተመረጠም!') : '⏳ የምርጫ ጊዜ፡ ' + selectionTime + ' ሰኮንድ'}
                  </div>
                </div>

                {/* Main Content (Vertical Layout - Board on top, Wheel section at bottom) */}
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '10px', padding: '0 8px 8px 8px', overflowY: 'auto' }}>
                  
                  {/* Selected Numbers Box */}
                  <div style={{ backgroundColor: '#1b1b32', borderRadius: '8px', padding: '8px 12px', border: '1px solid #312e81', flexShrink: 0 }}>
                    <div style={{ fontSize: '11px', color: '#38bdf8', marginBottom: '4px', fontWeight: 'bold' }}>
                      📌 የተመረጡ ቁጥሮች ({selectedNumbers.length}):
                    </div>
                    <div style={{ fontSize: '11px', color: '#9ca3af', lineHeight: '1.3', wordBreak: 'break-word', maxHeight: '50px', overflowY: 'auto' }}>
                      {selectedNumbers.length > 0 ? selectedNumbers.join(', ') : 'እስካሁን ምንም አልመረጡም'}
                    </div>
                  </div>

                  {/* Numbers Grid Container (1 to 1000 - Full Scrollable) */}
                  <div style={{ 
                    flex: 1, 
                    minHeight: '260px',
                    display: 'grid', 
                  ...
                  {/* Draw Spin Wheel Section */}
                  <div style={{ backgroundColor: '#13132b', borderRadius: '12px', padding: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '1px solid #23234d', flexShrink: 0 }}>
                    <div style={{ fontSize: '13px', fontWeight: 'bold', marginBottom: '12px', color: '#f59e0b' }}>
                      🎰 የእጣ ማውጫ
                    </div>
                    <div style={{
                      width: '120px',
                      ...
                      {winningNumber === 'SPINNING' ? (
                        ...
                        <span style={{ fontSize: '12px', color: '#ef4444', fontWeight: 'bold', textAlign: 'center' }}>አልተመረጠም</span>
                      ) : (
                        <span style={{ fontSize: winningNumber === '?' ? '46px' : '40px', fontWeight: 'bold', color: winningNumber === '?' ? '#ffffff' : '#00ffcc', textShadow: winningNumber === '?' ? 'none' : '0 0 12px #00ffcc' }}>
                          {winningNumber}
                        </span>
                      )}
                    </div>
                    ...
                    {winnerInfo && (
                      <div style={{ marginTop: '12px', width: '100%', padding: '12px', backgroundColor: '#064e3b', border: '2px solid #10b981', borderRadius: '10px', textAlign: 'center', boxShadow: '0 0 15px rgba(16, 185, 129, 0.4)' }}>
            ...
            {walletTab === 'balance' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div style={{ backgroundColor: '#181830', padding: '16px', borderRadius: '12px', border: '1px solid #2a2a4a' }}>
                  ...
                  <button onClick={handleDeposit} style={{ width: '100%', padding: '12px', backgroundColor: '#22c55e', color: '#fff', border: 'none', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}> የገቢ ጥያቄ ላክ (Submit Deposit) </button>
                </div>
                <div style={{ backgroundColor: '#181830', padding: '16px', borderRadius: '12px', border: '1px solid #2a2a4a' }}>
                  ...
                  <button onClick={handleWithdraw} style={{ width: '100%', padding: '12px', backgroundColor: '#ef4444', color: '#fff', border: 'none', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}> የወጪ ጥያቄ ላክ (Submit Withdraw) </button>
                </div>
              </div>
            )}
          </div>
        ...
        {isAdmin && currentTab === 'admin' && (
          <div style={{ flex: 1, padding: '20px 16px', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
                  ...
                  {filteredAdminUsers.map((u) => (
                    <div key={u.userId} style={{ backgroundColor: '#181830', border: '1px solid #2a2a4a', borderRadius: '10px', padding: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        <button onClick={() => { setEditingUser(u); setEditMain(u.mainWallet); setEditPlay(u.playWallet); }} style={{ backgroundColor: '#0284c7', color: '#fff', border: 'none', borderRadius: '6px', padding: '6px 10px', fontSize: '11px', cursor: 'pointer', fontWeight: 'bold' }}> አስተካክል </button>
            ...
            {adminTab === 'game_control' && (
              <div style={{ backgroundColor: '#181830', padding: '16px', borderRadius: '12px', border: '1px solid #2a2a4a' }}>
                <h3 style={{ fontSize: '15px', color: '#f59e0b', marginTop: 0 }}>🎯 የእጣ ቁጥር ማውጣት መቆጣጠሪያ (Draw Control)</h3>
                <p style={{ fontSize: '11px', color: '#9ca3af' }}>የሚቀጥለው የእጣ ቁጥር በራስ-ሰር (Random) ሳይሆን እርሶ በሚመርጡት ቁጥር እንዲወጣ ማድረግ ይችላሉ።</p>
